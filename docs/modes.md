...
[подписи]          финальная отрисовка подписей поверх уникализированного видео
      │
      ▼
[гейт] QA layer    проверка формата: 1080×1920 · 30fps · есть звук
      │
      ▼
...
`qa_meme` автоматически проверяет только **формат** (размер 1080×1920, fps 29–31,
наличие звуковой дорожки). Стык лицо→панч и читаемость подписей проверяются
**визуально** по кадрам на шаге рендера; дедуп-дистанция (vpdq) — будущее
дополнение.
