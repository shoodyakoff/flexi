...
1. **`generated_video`** — «Broll-рилс с ElevenLabs» под ключ: хук + озвучка основной части + CTA, с
...
> **Пользовательские названия режимов (что говорит пользователь) → маршруты этого
> файла.** Пользователь говорит по-русски; фразы-триггеры см. в `CLAUDE.md`.
> - **Broll-рилс с ElevenLabs** → стандартный режим (`generated_video` + library auto/manual);
> - **Говорящая голова** → чистая динамичная нарезка talking-head (раздел 5);
> - **Динамичный рилс** → маршрут 3-strip (`pipelines/three_strip/`);
> - **Демо продукта** и **Много рилсов** — отдельные режимы *в разработке*
>   (ref-style `render_ref_style_directed.py` и Reel Matrix `reel_matrix.py`); не
>   путать с маршрутами «кастомная графика» / «гибридный» ниже.
