...
def split_hook_cta(
    ...
    `words` is a list of (start, end, text) sorted by start. A run breaks
    wherever the silence between consecutive words exceeds `gap_thresh`
    seconds. Returns ((hook_start, hook_end), (cta_start, cta_end)):
    - hook = the span of the first contiguous run (up to its trailing gap).
    - cta = the span from the start of the next run through the end of the
      last word — i.e. "everything after the hook", not strictly the single
      last run. This matters when the body itself contains a pause bigger
      than `gap_thresh` (e.g. a breath between sentences): that pause must
      not slice the real CTA away from the rest of the tail.
    ...
    # single last run: a real take's body can contain its own pause > thresh
    # (e.g. a mid-sentence breath), which would otherwise split the CTA away
    # from the rest of the tail. Merge all post-hook runs into one span.
    tail = runs[1:] or runs
